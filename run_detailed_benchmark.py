...
from run_benchmark import patch_tokenizer_config, check_hf_access
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(os.path.join(config.LOG_DIR, "detailed_execution.log"), mode="a")
    ]
)
logger = logging.getLogger("DetailedBenchmark")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Detailed MTP Simulation Suite for different Gemma 4 models")
    parser.add_argument(
        "--size",
        type=str,
        default="e2b",
...
def run_single_inference(tokenizer, model, prompt_text, device, max_tokens, assistant_model=None):
    ...
    # Main generation
    profiler_label = "detailed_mtp" if assistant_model is not None else "detailed_baseline"
...
def main():
    ...
    results_md_path = f"/Users/pank/Experiments/MTP/results_detailed_{size}.md"

    logger.info("=========================================")
    logger.info(f"Initializing Gemma 4 DETAILED Simulation Suite for size: {size.upper()}")
    ...
    for test_idx, item in enumerate(DETAILED_PROMPTS):
        ...
        logger.info(f"[{test_idx + 1}/{len(DETAILED_PROMPTS)}] Running Detailed Scenario: {p_desc}")
    ...
    logger.info(f"Generating detailed simulation report at: {results_md_path}")
    ...
    logger.info(f"Detailed simulation complete for Gemma 4 {size.upper()} and results successfully saved.")
